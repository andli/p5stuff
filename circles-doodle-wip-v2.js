...
const NUM_CIRCLES = 1100;
const CIRCLE_MARGIN = 3;
const CIRCLE_MIN_RADIUS = 5;
const CIRCLE_MAX_RADIUS = 91;
const CIRCLE_FIRST_RADIUS = 91;
const CIRCLE_FILTER_RADIUS = 25;
...
function draw() {
  ...
  function createRelativeCircle(originCircle, angle, progressValue) {
    ...
    let distanceFromCenterFactor =
      Math.hypot(originCircle[0] - width / 2, originCircle[1] - height / 2) /
      (width / 2);
    // print(distanceFromCenterFactor);
    let adjustedMaxRadius = CIRCLE_MAX_RADIUS * (1 - distanceFromCenterFactor);
    // let adjustedMaxRadius = CIRCLE_MAX_RADIUS * (1 - progressValue);
    // print(adjustedMaxRadius);
    let r = random(CIRCLE_MIN_RADIUS, adjustedMaxRadius + CIRCLE_MIN_RADIUS);
